...
以下資料**僅儲存在您的瀏覽器本地** (`chrome.storage.local` 與 `localStorage`)：

| 資料類型 | 用途 | 加密狀態 |
|----------|------|----------|
| API 金鑰 | 調用 AI 提供商 API | ✅ AES-GCM + Web Crypto API 加密 |
| 對話歷史 | 顯示歷史聊天記錄 | ✅ 加密 |
| 設定偏好 | 模型選擇、主題、語言等 | ✅ 加密 |
...
- **金鑰生成**：Web Crypto API `crypto.subtle.generateKey()` 隨機生成
- **金鑰儲存**：localStorage (Base64 編碼原始金鑰)
- **初始向量**：每次加密獨立隨機產生 (12 bytes)
- **瀏覽器綁定**：金鑰綁定當前瀏覽器設定檔，換裝置/瀏覽器自動失效

> **重大變更 (v1.0.0)**：移除主密碼機制。金鑰現在由瀏覽器自動生成並綁定，無需使用者記住或輸入密碼。資料自動加密，使用體驗零摩擦。

### 3.2 無主密碼設計

- ✅ **無需設定密碼** - 首次使用直接進入應用
- ✅ **零摩擦體驗** - 資料自動加密，無感知
- ✅ **裝置級安全** - 金鑰綁定瀏覽器設定檔，換裝置自動失效
- ✅ **無密碼遺忘風險** - 無主密碼即無忘記密碼問題
- ✅ **重置功能保留** - 仍可透過「重置所有資料」清除所有加密資料
